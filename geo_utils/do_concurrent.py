...
def smarty_process_thread(
    df: pd.DataFrame, smartystreets_auth_id: str, smartystreets_auth_token: str
) -> pd.DataFrame:
    df_copied = df.copy()
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ss_results = pd.DataFrame.from_dict(
            dict(
                tqdm(
                    executor.map(smartystreets_utils.smarty_api, df_tolist),
                    total=len(df_tolist),
        ...
        combined_output = pd.concat(
            [df.reset_index(drop=True), ss_results], axis=1, ignore_index=True
        )  # concat strips colnames
        combined_output.columns = list(df.columns) + list(
            ss_results.columns
        )  # combines original cols
